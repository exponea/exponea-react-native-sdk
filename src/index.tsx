...
import { SdkAuthErrorCode } from './SdkAuthError';
 ...
 *
 * ### Interface A: TurboModule Methods
 *
...
export interface ExponeaType {
   ...
   *  Optionally accepts a `CustomerIdentity` so initial customer IDs and Stream JWT can be applied during init.
   */
  configure(
   ...
   * @deprecated Use `identifyCustomer(customerIdentity, properties?)` instead.
   */
  identifyCustomer(
  ...
  /**
   * Identifies current customer using a `CustomerIdentity` (customer IDs +
   * optional Stream JWT). `properties` is optional and defaults to an empty map.
   */
  identifyCustomer(
    customerIdentity: CustomerIdentity,
    properties?: JsonObject
   ...
   * Optionally changes default Exponea project and event routing.
   * @deprecated Use `anonymize(integrationConfig?, integrationRouteMap?)` instead.
   */
  anonymize(
  ...
  /**
   * Anonymizes the current customer and creates a new anonymous one.
   * The push token is cleared on the Exponea backend.
   *
   * Optionally switches the default configuration integration to a different
   * `ProjectConfig` or `StreamConfig`, and overrides the per-event-type
   * project routing map.
   *
   * Calling `anonymize()` with no arguments keeps the existing integration
   * configuration and simply resets the customer identity.
   *
   * @param integrationConfig - Optional new integration to use after anonymization.
   *   Pass a `ProjectConfig` for a standard project or a `StreamConfig` for a
   *   stream-based integration. If omitted, the current integration is retained.
   * @param integrationRouteMap - Optional per-event-type routing overrides.
   *   Only applicable when `integrationConfig` is a `ProjectConfig`; ignored
   *   for `StreamConfig`.
   */
  anonymize(
  ...
  /** Sets a Stream JWT auth token. Only effective with StreamConfig integrations. */
  ...
  /**
   * Registers a callback called when the native SDK reports an auth-token error.
   * @see ExponeaListeners.setSdkAuthErrorCallback
   */
  setSdkAuthErrorCallback(callback: (error: SdkAuthError) => void): void;
  /**
   * Removes the previously registered auth callback handler.
   * @see ExponeaListeners.removeSdkAuthErrorCallback
   */
...
export const setSdkAuthToken = Exponea.setSdkAuthToken;
...
export const setSdkAuthErrorCallback = Exponea.setSdkAuthErrorCallback;
