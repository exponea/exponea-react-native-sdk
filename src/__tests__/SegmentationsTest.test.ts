...
describe('parameter serialization and typings', () => {
  ...
  test('Segmentation callback registration', () => {
    ...
    expect(mockExponea.lastArgumentsJson).toBe(
      `{"exposingCategory":"discovery","includeFirstLoad":true}`
    );
  });
  ...
  test('Segmentation manual fetch without force', () => {
    ...
    expect(JSON.parse(mockExponea.lastArgumentsJson)).toStrictEqual(
      JSON.parse(
        TestUtils.readJsonFile('./src/test_data/get-segments-nonforced.json')
      )
    );
  });
  ...
  test('Segmentation manual fetch with force', () => {
    ...
    expect(JSON.parse(mockExponea.lastArgumentsJson)).toStrictEqual(
      JSON.parse(
        TestUtils.readJsonFile('./src/test_data/get-segments-forced.json')
      )
    );
  });
  ...
  test('Segmentation manual fetch without force param', () => {
    ...
    expect(JSON.parse(mockExponea.lastArgumentsJson)).toStrictEqual(
      JSON.parse(
        TestUtils.readJsonFile('./src/test_data/get-segments-minimal.json')
      )
    );
  });
});
